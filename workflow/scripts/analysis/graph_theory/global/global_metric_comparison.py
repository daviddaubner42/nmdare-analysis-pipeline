...
props = pd.read_table(args.metric_path)
...
# Save results
with open(os.path.join(args.out_dir, "task-rest_space-fsLR_seg-DesikanKilliany_den-91k_desc-globalMetricPvals_relmat.pkl"), "wb") as f:
    pickle.dump(ps, f)
with open(os.path.join(args.out_dir, "task-rest_space-fsLR_seg-DesikanKilliany_den-91k_desc-globalMetricStats.pkl"), "wb") as f:
    pickle.dump(stats, f)
...
sidecar["Sources"] = [f"bids:graph_theory/global:task-rest_space-fsLR_seg-DesikanKilliany_den-91k_desc-globalMetrics_relmat.tsv"]

with open(os.path.join(args.out_dir, "task-rest_space-fsLR_seg-DesikanKilliany_den-91k_desc-globalMetricPvals_relmat.json"), "w") as f:
    json.dump(sidecar, f)
with open(os.path.join(args.out_dir, "task-rest_space-fsLR_seg-DesikanKilliany_den-91k_desc-globalMetricStats.json"), "w") as f:
    json.dump(sidecar, f)