...
parser = argparse.ArgumentParser(description="Calculate the average shortest path length for each ROI for each density")
...
for gd in np.arange(args.min_density, args.max_density + args.density_step, args.density_step):
    ...
    pls = dict(nx.shortest_path_length(G))
    aspls = {}
    for area in G.nodes:
        aspls[area] = np.mean(list(pls[area].values()))
    
    for area, metric in aspls.items():
