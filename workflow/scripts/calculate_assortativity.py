...
parser = argparse.ArgumentParser(description="Calculate the assortativity for each subject's FC matrix")
...
for gd in np.arange(args.min_density, args.max_density + args.density_step, args.density_step):
    ...
    metric_per_density[gd] = nx.degree_assortativity_coefficient(G)

# Save the assortativity for each density in a pickle file
