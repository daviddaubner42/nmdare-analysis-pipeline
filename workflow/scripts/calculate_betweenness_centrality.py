...
parser = argparse.ArgumentParser(description="Calculate the betweenness centrality for each ROI for each density")
...
for gd in np.arange(args.min_density, args.max_density + args.density_step, args.density_step):
    ...
    metrics = nx.betweenness.betweenness_centrality(G)
