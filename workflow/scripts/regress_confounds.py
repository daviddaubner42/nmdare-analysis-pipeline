...
for subid in args.subids:
    ...
    fc = np.loadtxt(os.path.join(args.fc_dir, f"sub-{subid}", f"sub-{subid}_FC_raw.csv"), delimiter=',')
    fcs.append(fc)
...
for i, subid in enumerate(args.subids):
    np.savetxt(os.path.join(args.fc_dir, f"sub-{subid}", f"sub-{subid}_FC_regr.csv"), fcs[i], delimiter=',')