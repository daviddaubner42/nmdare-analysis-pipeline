...
parser.add_argument("--subids", type=str, help="All the subids to be included")
...
for subid in args.subids:
    fcd = np.loadtxt(os.path.join(args.fcds_path, f"sub-{subid}", f"sub-{subid}_FCD.csv"), delimiter=',')
    with open(os.path.join(args.fcds_path, f"sub-{subid}", f"sub-{subid}_FCD_hist.pkl"), "rb") as f:
        fcd_hist = pickle.load(f)
    if demo_data[demo_data["ID"] == subid]["nmdare"].item() == 1:
        nmdare_fcds.append(fcd)
        nmdare_hists.append(fcd_hist)
    else:
        hc_fcds.append(fcd)
        hc_hists.append(fcd_hist)
...
fig.savefig(f"{args.fcds_path}/images/avg_FCD_comparison.png", bbox_inches="tight")
...
fig.savefig(f"{args.fcds_path}/images/avg_hist_comparison.png", bbox_inches="tight")