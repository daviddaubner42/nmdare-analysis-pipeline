...
parser = argparse.ArgumentParser(description="Calculate the global efficiency for each subject's FC matrix")
...
for gd in np.arange(args.min_density, args.max_density + args.density_step, args.density_step):
    ...
    metric_per_density[gd] = nx.global_efficiency(G)

# Save the global efficiency for each density in a pickle file
