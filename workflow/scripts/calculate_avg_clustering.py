...
parser = argparse.ArgumentParser(description="Calculate the average clustering coefficient for each subject's FC matrix")
...
for gd in np.arange(args.min_density, args.max_density + args.density_step, args.density_step):
    ...
    metric_per_density[gd] = nx.average_clustering(G)

# Save the average clustering coefficient for each density in a csv file
